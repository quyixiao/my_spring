...
public abstract class AbstractBeanDefinitionReader implements EnvironmentCapable, BeanDefinitionReader {
    ...
    public int loadBeanDefinitions(String location, Set<Resource> actualResources) throws BeanDefinitionStoreException {
        ...
        if (resourceLoader instanceof ResourcePatternResolver) {
            ...
            try {
                // 将指定的位置的Bean配置信息解析为Spring Ioc容器封装的资源
                // 加载多个指定位置的Bean的配置信息
                Resource[] resources = ((ResourcePatternResolver) resourceLoader).getResources(location);

                // 委派调用其子类的XmlBeanDefinitionReader的方法实现加载功能
                int loadCount = loadBeanDefinitions(resources);
                log.info(" loadBeanDefinitions arrays " + Arrays.toString(resources) + " , count = " + loadCount);
            ...
            // 将指定位置的Bean的配置信息解析为Spring IoC容器封装的资源
            // 加载单个指定位置的Bean配置信息
            ...
            log.info("Loaded " + loadCount + " bean definitions from location [" + location + "]");

            return loadCount;
        }
    }

    // 重载方法，调用loaderBeanDefinitions(String) 方法
    @Override
