...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
    ...
    public PathMatchingResourcePatternResolver() {
		log.info(" resourceLoader init by DefaultResourceLoader no params");
    ...
    public PathMatchingResourcePatternResolver(ResourceLoader resourceLoader) {
		...
		LoggerUtils.info(" resourceLoader init by resourceLoader",9);
    ...
    public PathMatchingResourcePatternResolver(ClassLoader classLoader) {
		log.info(" resourceLoader init by DefaultResourceLoader params is classLoader");
    ...
    public ResourceLoader getResourceLoader() {
    	log.info(" ResourceLoader getResourceLoader");
    ...
    public Resource[] getResources(String locationPattern) throws IOException {
        log.info("getResources locationPattern : " + locationPattern);
        Assert.notNull(locationPattern, "Location pattern must not be null");
        if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX)) {
            log.info("locationPattern file startWith classpath* ");
            ...
            String prefixEndStr = locationPattern.substring(prefixEnd);
            log.info("locationPattern prefixEndStr : {}", prefixEndStr);

            if (pathMatcher.isPattern(prefixEndStr)) {
            	log.info("pathMatcher isPattern {}" + prefixEndStr);
                // a file pattern
                return findPathMatchingResources(locationPattern);
            } else {
				log.info("pathMatcher is not Pattern {}" , prefixEndStr);
                // a single resource with the given name
				ResourceLoader resourceLoader = getResourceLoader();
				log.info("resourceLoader name is  {}" , resourceLoader.getClass().getName());
